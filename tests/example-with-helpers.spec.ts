...
import {urls,forGoogle} from "../lib/helpers/text-helpers";
const { BASE_WORD } = forGoogle;
...
describe("Check helpers", () => {
  ...
  it("The 'Clear and type' function check", async ({ page }) => {
    await followToPage(urls.GOOGLE_URL, page);
  ...
  it("Check for upload file", async ({ page }) => {
    await followToPage(urls.DROPFILES_SITE, page, "load");
    await uploadFiles(
  ...
  it("Check Snapshot making", async ({ browserName, page }) => {
    await followToPage(urls.EXAMPLE_SITE, page);
